...
%       - 'MY_ICC' - Intraclass Correlation Coefficient
%     {default = {'eq';'r2';'SSE';'n'} }
%
...
%      - 'kw' - Kruskal-Wallis test % Habibur
...
%      - 'Absolute' - same units as the data {default} % Habibur
...
% default values
params.corrInfo = {'eq';'p';'RMSE';'r2';'n'}; % Habibur
...
params.baStatsMode = 'Normal'; % Habibur
...
[r, LB, UB, F, df1, df2, p] = ICC([data.maskedSet1, data.maskedSet2], '1-1', 0.05, 0.5);
stats.MY_ICC = r;
...
for i=1:length(params.corrInfo)
	switch upper(params.corrInfo{i})
		...
		case 'RHO (P)', corrtext = [corrtext; ['rho=' mynum2str(stats.rho,4,4) ' (p=' num2str(stats.rhoP) ')']];
        ...
        case 'MY_ICC', corrtext = [corrtext; ['ICC=' mynum2str(stats.MY_ICC,4,4)]];
	end
end
...
switch upper(params.diffValueMode)
	case 'ABSOLUTE'
        ...
        data.maskedDifferencesRelative = data.maskedDifferences ./ data.maskedSet2;
...
stats.differenceMAE = sum(abs(data.maskedDifferences))/numel(data.maskedDifferences); % Habibur
stats.differenceMAPE = sum(abs(data.maskedDifferencesRelative)) *100/numel(data.maskedDifferences); % Habibur
...
stats.kwp = kruskalwallis([data.maskedSet1, data.maskedSet2], [], 'off'); % Habibur
...
for i=1:length(params.baInfo)
	switch upper(params.baInfo{i})
		...
		case 'KS' % Kolmogorov-Smirnov test that difference-data is Gaussian
			BAtext = [BAtext; ['KS p-value: ' num2str(stats.ksp)]];
        case 'KW' % p value of Kruskal-Wallist t test
            % BAtext = [BAtext; ['KW p-value: ' mynum2str(stats.kwp,10,10)]];
            BAtext = [BAtext; ['KW p-value: ' num2str(stats.kwp)]];
        ...
        case 'MAPE'
            BAtext = [BAtext; ['MAPE: ' mynum2str(stats.differenceMAPE,2,2)]];
        case 'MAE'
            BAtext = [BAtext; ['MAE: ' mynum2str(stats.differenceMAE,2,2)]];
	end
end
a = axis(baAH);
